...
def main():
    with sync_playwright() as p:
        ...
        context.route("**/health", forward)
        result = context.request.get(API + "/api/analysis/" + FILE_ID, timeout=120000)
        ...
        page.screenshot(path="data/validation/semantic-dashboard-desktop.png", full_page=True)
        ...
        page.get_by_role("searchbox").fill(os.getenv("TEST_SEARCH", "navigation"))
        ...
        expect(page.get_by_role("heading", name="Speaker analytics", exact=True)).to_be_visible()
        page.go_back()
        ...
        for width in (768, 390):
            ...
            page.screenshot(path=f"data/validation/semantic-dashboard-{width}.png", full_page=True)
        ...
        page.reload()
        expect(page.get_by_text("There is not enough meaningful transcript content to summarize.")).to_be_visible(timeout=30000)
